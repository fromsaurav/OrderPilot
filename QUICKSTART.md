...
> **Which API the UI calls** — `NEXT_PUBLIC_API_BASE`, resolved in this order (first wins):
> 1. an inline shell var on the command (highest priority),
> 2. `frontend/.env.local` (currently set to the cloud API),
> 3. the code default `http://localhost:8000`.
>
> So plain `npm run dev` uses `.env.local` (cloud). To point at the **local** backend for a quick
> test, override inline — no need to edit `.env.local`:
> ```bash
> cd /home/saurav/Saurav/OrderPilot/frontend
> NEXT_PUBLIC_API_BASE=http://localhost:8000 npm run dev      # -> http://localhost:3000
> ```
> (The page header shows the API it's actually using — confirm it there.)
...
# 2. Run the UI pointed at the LOCAL backend (inline override beats frontend/.env.local)
cd ../frontend && npm install
NEXT_PUBLIC_API_BASE=http://localhost:8000 npm run dev      # -> http://localhost:3000

# 3. (optional) seed data so the UI isn't empty + prove the whole flow
./scripts/seed_demo.sh http://localhost:8000   # populate a few runs
./scripts/local_demo.sh                        # 17 assertions
#   Temporal Web UI: http://localhost:8080
```

To use the **Gemini** LLM path locally: put `GEMINI_API_KEY=<key>` in `backend/.env` (gitignored)
before `docker compose up`; a `customer_message_received` event then logs an `llm` line.

Stop it: `cd backend && docker compose down` (add `-v` to wipe the local Postgres volume).

> **Empty UI?** The page header shows which API it's hitting. If it shows a cloud IP (e.g.
> `65.2.4.186`) that you've since torn down, you're on `.env.local` — restart with the inline
> `NEXT_PUBLIC_API_BASE=http://localhost:8000` override above. Tearing down the **cloud** wipes its
> data (local-path PVs live on the EC2 nodes); `seed_demo.sh` repopulates on the next standup.
